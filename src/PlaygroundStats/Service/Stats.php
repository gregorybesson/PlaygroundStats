...
class Stats extends EventProvider implements ServiceManagerAwareInterface
{
	...
	{
		$em = $this->getServiceManager()->get('application_doctrine_em');
		...
		$sql = "SELECT distinct u.user_id, u.username, u.email, u.title, u.firstname, u.lastname, u.address, u.address2, u.postal_code, u.city, u.telephone, u.mobile, u.optin_partner, u.children, u.registration_source";
		...
		if($hardbounce != 'all'){
			$sql .= ', u.is_hardbounce, u.date_set_hardbounce';
		}
		...
		switch ($hardbounce){
			case 'yes' 		: $sql .= " AND u.is_hardbounce = 1"; break;
			case 'no' 		: $sql .= " AND u.is_hardbounce = 0"; break;
			case 'between' 	: $sql .= " AND u.is_hardbounce = 1 AND (u.date_set_hardbounce >= '" . $hardbounceStart->format('Y-m-d') . " 00:00:00' AND u.date_set_hardbounce <= '" . $hardbounceEnd->format('Y-m-d') . " 00:00:00')"; break;
		}
